...
class SettingsPreferences(private val application: Application) : ISettingsPreferences {
    ...
    override fun insert(settings: Settings) {
        ...
        sp.getString(ALL_DATA_ROUTE, null)
            ?.let { Companion.settings.postValue(settingsFromJson(it)) }
    }
    ...
    override fun getSettings(): Settings {
        return sp.getString(ALL_DATA_ROUTE, null)
            ?.let { settingsFromJson(it) } ?: Settings.getDefault()
    }
